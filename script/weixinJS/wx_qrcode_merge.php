...
// 通过openId获得member_id  数据库，取得会员memberID
...
// 二维码链接ticket下载图片到当前目录
$imageInfo = downImgFromQrcode($url);
$fileName = 'qrcode_'.$memberID.'.jpg';
...
if(localFile !== false){
	if(fwrite($localFile,$imageInfo['body']) !== false){
		...
		// 合并图片
		$qrcodeImg = imgMerge("qrcode_bg.jpg",$fileName);
		echo "<script>location.href='../../qrcode.html?$qrcodeImg';</script>";
	}
} 

// 合并图片，底图+二维码
function imgMerge($imgBg,$fileName){
  // 底图
  //$path_1 = "qrcode_bg.jpg";
  // 装备图片
  //$path_2 = $fileName;
  // 将人物和装备图片分别取到两个画布中
  $image_1 = imagecreatefromjpeg("qrcode_bg.jpg"); //imagecreatefromgif
  $image_2 = imagecreatefromjpeg($fileName);
  // 创建一个和人物图片一样大小的真彩色画布
  ...
  imagecopymerge($image_3,$image_2, 0,120,0,0,imagesx($image_2),imagesy($image_2), 100);
  // 将画布保存到指定的gif文件, 大功告成
  $fileMerge = "qrcode_".$memberID."_merge.jpg";
  imagejpeg($image_3, $fileMerge);
  return $fileMerge;
}
...
//从二维码链接下载图片保存到本地
function downImgFromQrcode($url)
{
	$ch = curl_init($url);
	curl_setopt($ch, CURLOPT_HEADER, 0);
